 ...
 *
 * @link       https://github.com/zachwatkins/tamus-order-plugin-wp/blob/master/templates/order-approval-template.php
...
function tamus_workstation_order_approval_scripts()
{
    ...
    wp_register_script(
        'tamus-order-plugin-wp-approval-scripts',
        TAMUS_ORDER_DIR_URL . 'js/order-approval.js',
        false,
        filemtime(TAMUS_ORDER_DIR_PATH . 'js/order-approval.js'),
        true
    );

    wp_enqueue_script('jquery');
    wp_enqueue_script('tamus-order-plugin-wp-approval-scripts');
 ...
 */
add_filter(
    'genesis_attr_entry-header', function ( $attr ) {
        $attr['class'] .= ' grid-x';
        return $attr;
    }
);
 ...
 */
add_filter(
    'genesis_attr_entry-title', function ( $attr ) {
