 ...
 *
 * @link       https://github.com/zachwatkins/tamus-order-plugin-wp/blob/master/js/order-approval.js
...
(function ($) {
    ...
    var validateLogisticsFields = function () {
        ...
        $acquisition_form.find('input[name*="req_number"],input[name*="req_date"]').each(
            function () {
                if (this.value === "") {
                    valid = false;
        ...
        jQuery.ajax(
            {
                ...
                success: function (data) {
                    if (data.indexOf("{") === 0) {
                        ...
                        $response.html(
                            '<span class="notice-red">There was an error confirming the order: ' + data + "</span>"
                        );
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    $response.html(
                        '<span class="notice-red">The application encountered an error while submitting your request (' +
                        errorThrown +
                        ").<br>" +
                        jqXHR.responseText +
                        "</span>"
        ...
        jQuery.ajax(
            {
                ...
                success: function (data) {
                    if (data.indexOf("{") === 0) {
                        ...
                        $approval_form
                        .parent()
                        .html('<span class="notice-red">You have returned the order to the end user.</span>');
                    } else {
                        $approval_form
                        .find(".ajax-response")
                        .html(
                            '<span class="notice-red">There was an error returning the order: ' + data + "</span>"
                        );
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {
    ...
    var ajaxLogistics = function (e, success_callback) {
        ...
        $acquisition_form.find(field_selector).each(
            function () {
                ...
                success: function (data) {
                    if (data.indexOf("{") === 0) {
                        ...
                        if ("success" === response.status) {
                            $acquisition_form
                            .find(".ajax-response")
                            .html('<div class="fade-out notice-green">You have updated the order.</div>');
                            window.setTimeout(
                                function () {
                                    $acquisition_form.find(".ajax-response .fade-out").fadeOut();
                                }, 3000
                            );
                            ...
                            $acquisition_form
                            .find(".ajax-response")
                            .html('<div class="notice-red">' + response.status + "</div>");
                        }
                    } else {
                        $acquisition_form
                        .find(".ajax-response")
                        .html(
                            '<span class="notice-red">There was an error updating the order: ' + data + "</span>"
                        );
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {
    ...
    var ajaxPublish = function (e) {
        ...
        if (true === valid) {
                    ...
                    success: function (data) {
                        if (data.indexOf("{") === 0) {
                            ...
                            if (response.hasOwnProperty("errors") 
                                ...
                                $acquisition_form
                                .find(".ajax-response")
                                .html('<span class="notice-red">' + output + "</span>");
                            } else {
                                $acquisition_form
                                .find(".ajax-response")
                                .html(
                                    '<span class="notice-green">You have published the order. The page will refresh in 3 seconds.</span>'
                                );
                                window.setTimeout(
                                    function () {
                                        location.reload();
                            ...
                            .find(".ajax-response")
                            .html(
                                '<span class="notice-red">There was an error publishing the order: ' +
                                    data +
                                    "</span>"
                            );
                        }
            ...
            $acquisition_form
                .find(".ajax-response")
                .html(
                    '<div class="flagged">Some of your fields are empty. You must provide information for them before you can publish the order.</div>'
                );
        }
    };
        ...
        jQuery.ajax(
            {
                ...
                success: function (data) {
                    if (data.indexOf("{") === 0) {
                        ...
                        if ("success" === response.status) {
                            $response.html(
                                '<span class="notice-green">You have reassigned the order to yourself. The page will refresh in 3 seconds.</span>'
                            );
                            $button.remove();
                            window.setTimeout(
                                function () {
                        ...
                        $response.html(
                            '<span class="notice-red">There was an error reassigning the order: ' + data + "</span>"
                        );
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {
